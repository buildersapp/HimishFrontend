<?php
include_once('admin/utils/helpers.php'); // or adjust path as needed
checkMaintenance();
...
function requireLoggedIn() {
    ...
    if (!isset($_SESSION['hm_wb_logged_in']) || $_SESSION['hm_wb_logged_in'] == false) {
        attemptAutoLogin();
    }
...
function redirectIfLoggedIn() {
    ...
    // If login successful (via session or cookie), redirect to home
...
function attemptAutoLogin() {
    if (isset($_COOKIE['pz_wb_user_auth'])) {
        $cookieData = json_decode($_COOKIE['pz_wb_user_auth'], true);
        ...
        if ($cookieData && isset($cookieData['user_id'], $cookieData['token'])) {
            ...
            if ($userId > 0) {
                ...
                if (!empty($decodedResponse['body'])) {
                    $_SESSION['hm_wb_timezone'] = $cookieData['timezone'] ?? 'UTC';
