...
if(isset($_POST['addPost'])){
    ...
    if(!empty($apiDataU['latitude']) && !empty($apiDataU['longitude'])){
        $apiDataU['post_locations'][] = array('latitude' => $apiDataU['latitude'], 'longitude' => $apiDataU['longitude'], 'community_id' => 0, 'country_code' => $apiDataU['country_code']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['excel'])) {
    ...
    if ($file['error'] == UPLOAD_ERR_OK) {
        $imageFilePath = $file['tmp_name'];
        $apiData['excel'] = new CURLFile($imageFilePath, $file['type'], $file['name']);
    }

    // Send file to API
    $response = sendCurlRequest(BASE_URL . '/admin-import-deals', 'POST', $apiData, [], true);
    $decodedResponse = json_decode($response, true);
    //dump($decodedResponse);
...
if (isset($_GET['tp']) && $_GET['tp'] == "export") {
    try {
        ...
        $response = sendCurlRequest(BASE_URL . '/admin-export-deals' . $query_data, 'POST', $apiData);
        ...
        foreach ($rawData as $item) {
            if (is_array($item)) {
                $data[] = [
                    "CREATED"       => formatToUSDate($item[15],1) ?? '',
                    "ID"            => $item[0] ?? '',
                    "User ID"       => $item[1] ?? '',
                    "Title"         => $item[2] ?? '',
                    "Description"   => $item[3] ?? '',
                    "Sale Price"    => $item[4] ?? '',
                    "Regular Price" => $item[5] ?? '',
                    "Anywhere"      => $item[6] ?? '',
                    "Address"       => $item[7] ?? '',
                    "City"          => $item[8] ?? '',
                    "State"         => $item[9] ?? '',
        ...
        $fileName = "Deals_" . date('YmdHis') . ".csv";
        ...
        if (!empty($data)) {
            fputcsv($output, array_keys($data[0])); // Use first row's keys as headers
        }
