var companyTable = $("#walletTable").DataTable({
...
sorting: false,
ajax: {
url: "ajax.php?action=get_wallet_history", // JSON datasource for companies
...
complete: function(data) {
    ...
    if (data.responseJSON && data.responseJSON.recordsTotal === 0) {
        $("#walletTable_wrapper").hide();
        $("#noDataMessage").show();
    } else {
        $("#walletTable_wrapper").show();
        $("#noDataMessage").hide();
    }
...
function toggleDeleteButton() {
    ...
    $("#walletDeleteButton").toggle(anyChecked); // Show if any checkbox is checked, hide otherwise
}
...
function deleteWalletHistory(selectedIds) {
    ...
    $.ajax({
        url: "ajax.php?action=delete_wallet_history",
        type: "POST",
...
$("#walletDeleteButton").on("click", function () {
